...
__all__ = ['erste fenster_panedwindow.py']
...
def code_der_in_datei_geschrieben_wird(liste=[],parameterliste=[],beschreibung=beschreibung,minwidth=int,minhight=int,
    ...
    master2=PanedWindow(master,orient=VERTICAL,sashpad=1, relief=RIDGE,showhandle=False)
    master2.pack(fill="both",expand=True)

    top_frame2=Frame(master2)
    bot_frame2=Frame(master2,width=0,height=0)

    master2.add(top_frame2,sticky=N,width=10000)
    master2.add(bot_frame2,sticky=NSEW,width=10000)


    top_frame=Frame(top_frame2,height=0,width=0,takefocus=0)
    top_frame.pack(side="top",anchor="n",expand=True,fill="both")


    arbeits_menü = Frame(bot_frame2,takefocus=0,width=0,height=0)
    arbeits_menü.pack(fill="x",side="bottom",anchor="s",expand=False)

    arbeits_fenster=Frame(bot_frame2,width=0,height=0)
    ...
    parameterliste_label.pack(anchor="se",fill="both",expand=True,side="left")
    parameterliste_label.insert("1.0","parameterliste = "+str(parameterliste))
